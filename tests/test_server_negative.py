...
def test_join_nonexistent_game(client):
    rv = client.post(
        "/join_game", json={"game_id": "no-such", "player_name": "X"})
...
def create_started_game_two_clients():
    ...
    assert c1.post(
        "/join_game", json={"game_id": gid, "player_name": "Alice"}).get_json()["success"]
    assert c2.post(
        "/join_game", json={"game_id": gid, "player_name": "Bob"}).get_json()["success"]
...
def test_duplicate_player_name(client):
    ...
    gid = rv.get_json()["game_id"]
    assert client.post(
        "/join_game", json={"game_id": gid, "player_name": "Same"}).get_json()["success"]
    r2 = client.post(
        "/join_game", json={"game_id": gid, "player_name": "Same"})
    data2 = r2.get_json()
...
def test_reroll_and_end_turn_wrong_player():
    ...
    players = st1["players"]
    alice_current = any(p["name"] == "Alice" and p["is_current"]
                        for p in players)
